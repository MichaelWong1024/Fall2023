...
create procedure recycle_crew (in ip_flightID varchar(50))
sp_main: begin
    -- Check if the flight is completed
    declare v_completed boolean default false;
    select (progress = 100 and airplane_status = 'Ready') into v_completed
    from flight
    where flightID = ip_flightID;

    if v_completed then
        -- Release pilots from the flight
        update pilot
        set commanding_flight = null
        where commanding_flight = ip_flightID;
        -- Add more statements here if there are other crew members or tasks to handle
    end if;
end //
delimiter ;
...
-- ) as
-- select 
--     a.airport_name as departing_from,
--     b.airport_name as arriving_at,
...
--     group_concat(distinct concat(a2.airlineID, '-', a2.tail_num)) as airplane_list
-- from 
--     flight f
--     join route_path rp on f.routeID = rp.routeID
--     join leg l on rp.legID = l.legID
--     join airport a on l.departure = a.airportID
--     join airport b on l.arrival = b.airportID
--     join airplane a2 on f.support_airline = a2.airlineID and f.support_tail = a2.tail_num
-- where 
--     f.airplane_status = 'in_air'
-- group by 
