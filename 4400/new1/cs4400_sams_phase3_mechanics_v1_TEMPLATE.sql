...
-- sp_main: begin
--     declare flight_ended int default 0;
...
--     and progress = max_sequence;

--     if flight_ended > 0 then
--         update pilot
...
BEGIN
    DECLARE flight_ended INT DEFAULT 0;
    DECLARE max_sequence INT;
    DECLARE passengers_onboard INT DEFAULT 0;
    ...
    SELECT COUNT(*)
    INTO passengers_onboard
    FROM person
    JOIN passenger ON person.personID = passenger.personID
    WHERE locationID = (SELECT locationID FROM airplane WHERE tail_num = 
          (SELECT support_tail FROM flight WHERE flightID = ip_flightID))
      AND person.personID NOT IN (SELECT personID FROM pilot);

    IF flight_ended > 0 AND passengers_onboard = 0 THEN
        -- Update the pilot table to release the crew
        UPDATE pilot
        SET commanding_flight = NULL
        WHERE commanding_flight = ip_flightID;
